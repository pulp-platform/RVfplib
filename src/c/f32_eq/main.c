...
int main(int argc, char** argv) {
    ...
    while (ret != EOF) {
        ret = scanf("%X %X %d %s", &op0, &op1, &gold_res, flags);
        res = __eqsf2(op0, op1);
        // SoftFloat f32_add does exactly the opposite of libgcc __eqsf2
        // So, invert both libgcc and rvfplib results, for testing purposes
        res ^= 0x1;
